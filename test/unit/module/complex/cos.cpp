  ...
  <typename T>(T const& a0, T const& a1 )
{
  ...
  for(auto e : a0)
  {
    for(auto f : a1)
    {
      ...
      TTS_ULP_EQUAL(eve::cos(eve::complex<e_t>(e, f)),  z_t(std::cos(c_t(e, f))), 2);
      TTS_ULP_EQUAL(eve::pedantic(eve::cos)(eve::complex<e_t>(e, f)),  z_t(std::cos(c_t(e, f))), 2);
      TTS_ULP_EQUAL(eve::cos(eve::complex<e_t>(e, f)),  z_t(std::cos(c_t(e, f))), 2.0);
    }
  }
};
  ...
  <typename T>(T const& a0, T const& a1 )
{
  ...
  for(int i = 0; i !=  eve::cardinal_v<T>; ++i)
  {
    ce_t z(std::cos(c_t(a0.get(i), a1.get(i))));
    b.set(i, z);
  }
  ...
  <typename T>(eve::as<T>)
{
  ...
  auto tcosine= [](auto z){return eve::pedantic(eve::cosh)(eve::mul_i(z));};
  for(int i=0; i < N; ++i)
  {
