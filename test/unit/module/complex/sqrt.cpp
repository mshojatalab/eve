  ...
  <typename T>(T const& a0, T const& a1 )
{
  ...
  for(auto e : a0)
  {
    for(auto f : a1)
    {
      TTS_ULP_EQUAL(eve::sqrt(eve::complex<e_t>(e, f)),  z_t(std::sqrt(c_t(e, f))), 2);
//      TTS_ULP_EQUAL(eve::pedantic(eve::sqrt)(eve::complex<e_t>(e, f)),  z_t(std::sqrt(c_t(e, f))), 2);
      TTS_ULP_EQUAL(eve::sqrt(eve::complex<e_t>(e, f)),  z_t(std::sqrt(c_t(e, f))), 2.0);
    }
  }
};
  ...
  <typename T>(T const& a0, T const& a1 )
{
  ...
  using c_t = std::complex<e_t>;
  auto std_ch = [](auto x, auto y){return std::sqrt(c_t(x, y)); };
...
//   for(int i = 0; i !=  eve::cardinal_v<T>; ++i)
//   {
//     ce_t z(std::sqrt(c_t(a0.get(i), a1.get(i))));
//     b.set(i, z);
//   }
