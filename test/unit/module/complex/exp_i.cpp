  ...
  <typename T>(T const& a0, T const& a1 )
{
  ...
  for(auto e : a0)
  {
    for(auto f : a1)
    {
      TTS_ULP_EQUAL(eve::exp_i(eve::complex<e_t>(e, f)),  z_t(std::exp(c_t(-f, e))), 2);
      TTS_ULP_EQUAL(eve::exp_i(eve::complex<e_t>(e, f)),  z_t(std::exp(c_t(-f, e))), 2.0);
    }
  }
};
  ...
  <typename T>(T const& a0, T const& a1 )
{
  ...
  using c_t = std::complex<e_t>;
  auto std_ch = [](auto x, auto y){return std::exp(c_t(-y, x)); };
